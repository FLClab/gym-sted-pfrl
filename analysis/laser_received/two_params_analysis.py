"""
In this version the agent has two sets of params. One  is used at the start of the episode, until the agent
notices an increase in signal between acquisitions, when he will switch to the 2nd set of params
(monitoring vs acquiring)
"""
    ...
    for i in range(args.n_runs):
        ...
        while not done:
            if len(acqs) == 1:
                ...
                if numpy.sum(acqs[n_actions]) / numpy.sum(acqs[n_actions - 1]) >= 1.025:
                    a = param_set_acquiring
                else:
                    a = param_set_monitoring
