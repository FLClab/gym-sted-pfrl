"""
In this version the agent has a pre-set set of parameters that he will use to image during the whole episode
"""
...
def main():
    ...
    for i in range(args.n_runs):
        ...
        while not done:
            a = numpy.asarray([args.pdt, 5e-6, args.p_sted])
    ...
    if not os.path.exists(f"gym-sted-pfrl/analysis/laser_received/constant_params_pdt_{args.pdt}_psted_{args.p_sted}"):
        os.makedirs(f"gym-sted-pfrl/analysis/laser_received/constant_params_pdt_{args.pdt}_psted_{args.p_sted}")
    numpy.save(f"gym-sted-pfrl/analysis/laser_received/constant_params_pdt_{args.pdt}_psted_{args.p_sted}/laser_dose.npy", laser_received_per_episode)
    numpy.save(f"gym-sted-pfrl/analysis/laser_received/constant_params_pdt_{args.pdt}_psted_{args.p_sted}/gt_nb_nanodomains.npy", n_nanodomains_per_episode)
    numpy.save(f"gym-sted-pfrl/analysis/laser_received/constant_params_pdt_{args.pdt}_psted_{args.p_sted}/nb_nanodomains_id.npy", n_nanodomains_identified_per_episode)
    with open(f"gym-sted-pfrl/analysis/laser_received/constant_params_pdt_{args.pdt}_psted_{args.p_sted}/actions_per_run.txt", "w") as file:
